# -*- coding: utf-8 -*-
"""
HybridSCIMFP - 策略3变体：肌肉协同性特征 (n_synergies=5)
"""
import os
import sys
import numpy as np
...
class Config:
    ...
    OUTPUT_DIR = "./outputs/synergy_v1.0_n5"
    TARGET_LENGTH = 100
...
class SynergyFeatureExtractor:
    ...
    def __init__(self, n_synergies=5):  # 修改为5
...
class FeatureExtractor:
    def __init__(self, fs_emg=1000, fs_kin=100):
        self.fs_emg = fs_emg
        self.fs_kin = fs_kin
        self.synergy_extractor = SynergyFeatureExtractor(n_synergies=5)  # 修改为5
...
def train_synergy_n5():
    print("="*60)
    print("HybridSCIMFP - 策略3变体：n_synergies=5")
    print("="*60)
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_predictions.csv', index=False)
    ...
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_summary.csv', index=False)
