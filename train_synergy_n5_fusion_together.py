# -*- coding: utf-8 -*-
"""
HybridSCIMFP - Fusion + Together (n_synergies=5)
在层次化融合基础上叠加一致性损失
架构: EMG/KIN → 层次化融合 → 一致性损失 → FMA预测
"""
import os
import sys
import numpy as np
...
from fusion.hierarchical_fusion import HierarchicalFusionNetwork, ConsistencyLoss


class Config:
    DATA_DIR = r"C:\Users\Administrator\Desktop\openclaw_cloudcode\sci\dataset\Rocky_Data"
    OUTPUT_DIR = "./outputs/synergy_v1.0_n5_fusion_together"
    TARGET_LENGTH = 100
...
class FusionTogetherModel(nn.Module):
    """Fusion + Together 完整模型
    ...
    def forward(self, emg_seq, kin_seq):
        """
        参数:
            emg_seq: (B, T, 12) EMG序列
            kin_seq: (B, T, 63) KIN序列
        返回:
            dict: {
                'fma_score': (B, 1) FMA评分预测,
                'consistency_loss': 标量 一致性损失,
                'features': (B, hidden_dim*2) 融合特征,
            }
        """
        # Stage 1: 输入投影
        emg_feat = self.emg_proj(emg_seq)  # (B, T, hidden_dim)
        kin_feat = self.kin_proj(kin_seq)  # (B, T, hidden_dim)
        ...
        # Stage 3: FMA回归
        fma_score = self.fma_head(hier_features)
        ...
        # Stage 4: 一致性损失
        if "level_features" in hier_out:
            level_feats = hier_out["level_features"]
            if len(level_feats) >= 2:
                c_losses = []
                for i in range(len(level_feats) - 1):
                    lf1 = level_feats[i].mean(dim=1)
                    lf2 = level_feats[i + 1].mean(dim=1)
                    min_dim = min(lf1.shape[-1], lf2.shape[-1])
                    c_loss = self.consistency_loss(lf1[..., :min_dim], lf2[..., :min_dim])
                    c_losses.append(c_loss)
                outputs["consistency_loss"] = sum(c_losses) / max(1, len(c_losses))
        
        return outputs


class SynergyFeatureExtractor:
    """肌肉协同性特征提取器 - n_synergies=5"""
...
def load_and_extract_features():
    print("="*60)
    print("加载数据并提取特征（n_synergies=5 + Fusion + Together）...")
    print("="*60)
    ...
    subject_fma = {}
    subject_emg_kin = defaultdict(list)
    ...
    for sample in raw_samples:
        ...
        try:
            ...
            subject_emg_kin[sid].append((emg_aligned, kin_aligned))
            subject_fma[sid] = sample.fma_score
            
        except Exception as e:
            continue
    
    X, y, sids, emg_kin_data = [], [], [], []
    ...
    for sid, feats in subject_samples.items():
        if len(feats) > 0:
            ...
            # 聚合EMG和KIN数据
            emg_list = [ek[0] for ek in subject_emg_kin[sid]]
            kin_list = [ek[1] for ek in subject_emg_kin[sid]]
            emg_mean = np.mean(emg_list, axis=0)
            kin_mean = np.mean(kin_list, axis=0)
            emg_kin_data.append((emg_mean, kin_mean))
    ...
    return X, y, sids, emg_kin_data


def train_synergy_n5_fusion_together():
    print("="*60)
    print("HybridSCIMFP - n_synergies=5 + Fusion + Together")
    print("="*60)
    
    X, y, sids, emg_kin_data = load_and_extract_features()
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(loo.split(X_pca)):
        print(f"\nFold {fold_idx + 1}/{len(X_pca)}")
        
        X_train, X_test_fold = X_pca[train_idx], X_pca[test_idx]
        y_train, y_test = y[train_idx], y[test_idx]
        
        # 准备神经网络数据
        emg_train = np.array([emg_kin_data[i][0] for i in train_idx])
        kin_train = np.array([emg_kin_data[i][1] for i in train_idx])
        emg_test = np.array([emg_kin_data[i][0] for i in test_idx])
        kin_test = np.array([emg_kin_data[i][1] for i in test_idx])
        
        # 创建模型
        model = FusionTogetherModel(
        ...
        for epoch in range(Config.EPOCHS):
            emg_tensor = torch.FloatTensor(emg_train).to(Config.DEVICE)
            kin_tensor = torch.FloatTensor(kin_train).to(Config.DEVICE)
            y_tensor = torch.FloatTensor(y_train).unsqueeze(1).to(Config.DEVICE)
            
            optimizer.zero_grad()
            outputs = model(emg_tensor, kin_tensor)
            
            # 总损失 = MSE损失 + 一致性损失
            loss = criterion(outputs["fma_score"], y_tensor)
            if "consistency_loss" in outputs:
                loss = loss + 0.1 * outputs["consistency_loss"]
        ...
        model.eval()
        with torch.no_grad():
            emg_tensor = torch.FloatTensor(emg_test).to(Config.DEVICE)
            kin_tensor = torch.FloatTensor(kin_test).to(Config.DEVICE)
            
            outputs = model(emg_tensor, kin_tensor)
            nn_pred = outputs["fma_score"].squeeze().cpu().numpy()
        ...
        nn_pred_val = float(nn_pred) if np.isscalar(nn_pred) or nn_pred.ndim == 0 else float(nn_pred[0])
        w_svr, w_knn, w_gb, w_nn = 0.20, 0.18, 0.42, 0.20
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_together_predictions.csv', index=False)
    
    summary = pd.DataFrame([{
        'version': 'synergy_n5_fusion_together',
        'n_synergies': 5,
        'mae': mae,
        'acc_1': acc,
        'acc_pm1': acc_pm1,
    }])
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_together_summary.csv', index=False)
...
if __name__ == '__main__':
    result = train_synergy_n5_fusion_together()
