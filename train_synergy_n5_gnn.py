# -*- coding: utf-8 -*-
"""
HybridSCIMFP - 策略3变体 + GNN特征提取 (n_synergies=5)
集成train_v2.py的GNN特征提取器
"""
import os
import sys
import numpy as np
...
class GNNFeatureExtractorModule(nn.Module):
    """GNN特征提取模块 - 从train_v2.py集成"""
    
    def __init__(self, emg_dim=12, kin_dim=63, hidden_dim=64, num_gnn_layers=2, dropout=0.3):
        super().__init__()
        self.hidden_dim = hidden_dim
        
        # 多模态GNN
        self.gnn = MultiModalGNN(
            modalities=["emg", "kin"],
            input_dims={"emg": emg_dim, "kin": kin_dim},
    ...
    def forward(self, emg_seq, kin_seq):
        ...
        返回:
            fused_features: (B, T, hidden_dim) 融合特征
        """
        inputs = {"emg": emg_seq, "kin": kin_seq}
        outputs = self.gnn(inputs)
        return outputs["fused"]  # (B, T, hidden_dim)
...
class FeatureExtractor:
    def __init__(self, fs_emg=1000, fs_kin=100, use_gnn=True):
        self.fs_emg = fs_emg
        self.fs_kin = fs_kin
        ...
        if use_gnn:
            self.gnn_extractor = GNNFeatureExtractorModule(
                emg_dim=12, kin_dim=63, hidden_dim=Config.HIDDEN_DIM, num_gnn_layers=2, dropout=0.3
            ).to(Config.DEVICE)
            self.gnn_extractor.eval()
    ...
    def extract_gnn_features(self, emg, kin):
        """提取GNN特征"""
        with torch.no_grad():
            ...
            gnn_out = self.gnn_extractor(emg_tensor, kin_tensor)  # (1, T, hidden_dim)
            
            # 时间池化：均值和最大值
            gnn_mean = gnn_out.mean(dim=1).squeeze().cpu().numpy()  # (hidden_dim,)
            gnn_max = gnn_out.max(dim=1)[0].squeeze().cpu().numpy()  # (hidden_dim,)
            gnn_std = gnn_out.std(dim=1).squeeze().cpu().numpy()  # (hidden_dim,)
            
            return np.concatenate([gnn_mean, gnn_max, gnn_std])
...
def load_and_extract_features():
    print("="*60)
    print("加载数据并提取特征（n_synergies=5 + GNN）...")
    print("="*60)
    ...
    feature_extractor = FeatureExtractor(fs_emg=1000, fs_kin=100, use_gnn=True)
    ...
    for sample in raw_samples:
        ...
        try:
            ...
            # GNN特征
            gnn_feat = feature_extractor.extract_gnn_features(emg_aligned, kin_aligned)
            
            # 合并特征
            feat = np.concatenate([emg_feat, kin_feat, gnn_feat])
...
def train_synergy_n5_gnn():
    print("="*60)
    print("HybridSCIMFP - 策略3变体：n_synergies=5 + GNN")
    print("="*60)
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_gnn_predictions.csv', index=False)
    
    summary = pd.DataFrame([{
        'version': 'synergy_n5_gnn',
        'n_synergies': 5,
        'mae': mae,
        'acc_1': acc,
        'acc_pm1': acc_pm1,
    }])
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_gnn_summary.csv', index=False)
