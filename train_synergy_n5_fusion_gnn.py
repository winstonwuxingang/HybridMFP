# -*- coding: utf-8 -*-
"""
HybridSCIMFP - Fusion + GNN (n_synergies=5)
在层次化融合基础上叠加GNN特征提取
架构: EMG/KIN → GNN → 层次化融合 → 特征
"""
import os
import sys
import numpy as np
...
class Config:
    ...
    OUTPUT_DIR = "./outputs/synergy_v1.0_n5_fusion_gnn"
    TARGET_LENGTH = 100
...
class FusionGNNFeatureExtractor(nn.Module):
    """Fusion + GNN 特征提取器
    
    架构流程:
    1. GNN特征提取: 捕捉EMG和KIN的图结构关系
    2. 层次化融合: 多层次特征融合
    """
    ...
    def forward(self, emg_seq, kin_seq):
        ...
        """
        # Stage 1: GNN特征提取
        gnn_inputs = {"emg": emg_seq, "kin": kin_seq}
        gnn_outputs = self.gnn(gnn_inputs)
        
        emg_gnn_feat = gnn_outputs["emg"]  # (B, T, hidden_dim)
        kin_gnn_feat = gnn_outputs["kin"]  # (B, T, hidden_dim)
...
class FeatureExtractor:
    def __init__(self, fs_emg=1000, fs_kin=100, use_fusion_gnn=True):
        self.fs_emg = fs_emg
        self.fs_kin = fs_kin
        self.synergy_extractor = SynergyFeatureExtractor(n_synergies=5)
        self.use_fusion_gnn = use_fusion_gnn
        
        if use_fusion_gnn:
            self.fusion_gnn_extractor = FusionGNNFeatureExtractor(
                emg_dim=12, kin_dim=63, hidden_dim=Config.HIDDEN_DIM, dropout=0.3
            ).to(Config.DEVICE)
            self.fusion_gnn_extractor.eval()
    ...
    def extract_fusion_gnn_features(self, emg, kin):
        """提取 Fusion + GNN 特征"""
        with torch.no_grad():
            ...
            fusion_gnn_feat = self.fusion_gnn_extractor(emg_tensor, kin_tensor)
            return fusion_gnn_feat.squeeze().cpu().numpy()
...
def load_and_extract_features():
    print("="*60)
    print("加载数据并提取特征（n_synergies=5 + Fusion + GNN）...")
    print("="*60)
    ...
    feature_extractor = FeatureExtractor(fs_emg=1000, fs_kin=100, use_fusion_gnn=True)
    ...
    for sample in raw_samples:
        ...
        try:
            ...
            # Fusion + GNN 特征
            fusion_gnn_feat = feature_extractor.extract_fusion_gnn_features(emg_aligned, kin_aligned)
            
            # 合并特征
            feat = np.concatenate([emg_feat, kin_feat, fusion_gnn_feat])
...
def train_synergy_n5_fusion_gnn():
    print("="*60)
    print("HybridSCIMFP - n_synergies=5 + Fusion + GNN")
    print("="*60)
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_gnn_predictions.csv', index=False)
    
    summary = pd.DataFrame([{
        'version': 'synergy_n5_fusion_gnn',
        'n_synergies': 5,
        'mae': mae,
        'acc_1': acc,
        'acc_pm1': acc_pm1,
    }])
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_gnn_summary.csv', index=False)
...
if __name__ == '__main__':
    result = train_synergy_n5_fusion_gnn()
