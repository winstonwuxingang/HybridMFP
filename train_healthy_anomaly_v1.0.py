# -*- coding: utf-8 -*-
"""
策略1：异常检测方法 - train_healthy_anomaly_v1.0.py

核心思想：
- 将健康受试者视为"正常运动模式"
- 中风患者的运动偏离正常模式的程度反映其FMA等级
- 低FMA患者偏离程度更大
...
class Config:
    ...
    OUTPUT_DIR = "./outputs/healthy_anomaly_v1.0"
    TARGET_LENGTH = 100
...
def train_anomaly_v1():
    print("="*60)
    print("策略1：异常检测方法 v1.0")
    print("="*60)
    print("\n核心思想：")
    print("  - 健康受试者 = 正常运动模式")
    print("  - 中风患者偏离程度 = 异常分数")
    ...
    # 特征选择
    selector = SelectKBest(f_regression, k=min(50, X_stroke.shape[1]-1))
    X_stroke_selected = selector.fit_transform(X_stroke_scaled, y_stroke)
    X_healthy_selected = selector.transform(X_healthy_scaled)
    
    print(f"\n特征选择后维度: {X_stroke_selected.shape[1]}")
    ...
    ocsvm.fit(X_healthy_selected)
    
    # 方法2: IsolationForest
    iso_forest = IsolationForest(n_estimators=100, contamination=0.1, random_state=42)
    iso_forest.fit(X_healthy_selected)
    
    # 计算异常分数
    anomaly_ocsvm = ocsvm.decision_function(X_stroke_selected)
    anomaly_iso = iso_forest.decision_function(X_stroke_selected)
    
    # 异常分数归一化（负值表示异常，正值表示正常）
    anomaly_ocsvm_norm = (anomaly_ocsvm - anomaly_ocsvm.min()) / (anomaly_ocsvm.max() - anomaly_ocsvm.min() + 1e-10)
    anomaly_iso_norm = (anomaly_iso - anomaly_iso.min()) / (anomaly_iso.max() - anomaly_iso.min() + 1e-10)
    
    print(f"\n异常分数范围 (OCSVM): [{anomaly_ocsvm.min():.3f}, {anomaly_ocsvm.max():.3f}]")
    print(f"异常分数范围 (IsolationForest): [{anomaly_iso.min():.3f}, {anomaly_iso.max():.3f}]")
    
    # 分析异常分数与FMA的关系
    print("\n异常分数与FMA的关系:")
    print(f"{'FMA':<8} {'OCSVM均值':<15} {'ISO均值':<15}")
    print("-" * 40)
    for fma in sorted(set(y_stroke)):
        mask = y_stroke == fma
        print(f"{fma:<8.0f} {anomaly_ocsvm_norm[mask].mean():<15.3f} {anomaly_iso_norm[mask].mean():<15.3f}")
    ...
    for train_idx, test_idx in loo.split(X_stroke_selected):
        X_train, X_test = X_stroke_selected[train_idx], X_stroke_selected[test_idx]
        y_train, y_test = y_stroke[train_idx], y_stroke[test_idx]
        
        # 使用异常分数作为额外特征
        anomaly_train = anomaly_ocsvm_norm[train_idx]
        anomaly_test = anomaly_ocsvm_norm[test_idx]
        
        # 组合特征
        X_train_aug = np.column_stack([X_train, anomaly_train.reshape(-1, 1)])
        X_test_aug = np.column_stack([X_test, anomaly_test.reshape(-1, 1)])
        
        # 训练回归模型
        svr = SVR(kernel='rbf', C=1.0, epsilon=0.1)
        gb = GradientBoostingRegressor(n_estimators=50, max_depth=2, random_state=42)
        
        svr.fit(X_train_aug, y_train)
        gb.fit(X_train_aug, y_train)
        
        pred = 0.5 * svr.predict(X_test_aug)[0] + 0.5 * gb.predict(X_test_aug)[0]
    ...
    df = pd.DataFrame({
        ...
        'anomaly_score': anomaly_ocsvm_norm,
    })
    ...
    print(f"{'策略1 (异常检测)':<30} {mae:<10.2f} {acc:.0%}")
...
if __name__ == '__main__':
    mae, acc, acc_pm1 = train_anomaly_v1()
