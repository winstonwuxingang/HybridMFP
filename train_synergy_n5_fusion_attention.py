# -*- coding: utf-8 -*-
"""
HybridSCIMFP - Fusion + Attention (n_synergies=5)
在层次化融合基础上叠加时空注意力
架构: EMG/KIN → 层次化融合 → 时空注意力 → 特征
"""
import os
import sys
import numpy as np
...
class Config:
    ...
    OUTPUT_DIR = "./outputs/synergy_v1.0_n5_fusion_attention"
    TARGET_LENGTH = 100
...
class SpatioTemporalAttention(nn.Module):
    ...
    def forward(self, x):
        attn_out, _ = self.temporal_attention1(x, x, x)
        x = self.layer_norm1(x + self.dropout(attn_out))
        
        attn_out2, _ = self.temporal_attention2(x, x, x)
        x = self.layer_norm2(x + self.dropout(attn_out2))
        
        return x


class FusionAttentionFeatureExtractor(nn.Module):
    """Fusion + Attention 特征提取器
    ...
    def forward(self, emg_seq, kin_seq):
        ...
        返回:
            features: (B, hidden_dim * 2 * 3) 融合特征 (mean, max, std)
        """
        # Stage 1: 输入投影
        ...
        hier_features = hier_out["output"]  # (B, T, hidden_dim*2) or (B, hidden_dim*2)
        
        # 如果是样本维度，扩展为序列维度
        if hier_features.ndim == 2:
            seq_len = emg_seq.shape[1]
            hier_features = hier_features.unsqueeze(1).expand(-1, seq_len, -1)
        
        # Stage 3: 时空注意力
        attended = self.attention(hier_features)  # (B, T, hidden_dim*2)
        
        # 时间池化
        mean_pool = attended.mean(dim=1)  # (B, hidden_dim*2)
        max_pool = attended.max(dim=1)[0]  # (B, hidden_dim*2)
        std_pool = attended.std(dim=1)  # (B, hidden_dim*2)
        
        return torch.cat([mean_pool, max_pool, std_pool], dim=-1)  # (B, hidden_dim*6)
...
class FeatureExtractor:
    def __init__(self, fs_emg=1000, fs_kin=100, use_fusion_attention=True):
        self.fs_emg = fs_emg
        self.fs_kin = fs_kin
        ...
        if use_fusion_attention:
            self.fusion_attention_extractor = FusionAttentionFeatureExtractor(
                emg_dim=12, kin_dim=63, hidden_dim=Config.HIDDEN_DIM, dropout=0.3
            ).to(Config.DEVICE)
            self.fusion_attention_extractor.eval()
    ...
    def extract_fusion_attention_features(self, emg, kin):
        """提取 Fusion + Attention 特征"""
        with torch.no_grad():
            ...
            fusion_attn_feat = self.fusion_attention_extractor(emg_tensor, kin_tensor)
            return fusion_attn_feat.squeeze().cpu().numpy()
...
def load_and_extract_features():
    print("="*60)
    print("加载数据并提取特征（n_synergies=5 + Fusion + Attention）...")
    print("="*60)
    ...
    feature_extractor = FeatureExtractor(fs_emg=1000, fs_kin=100, use_fusion_attention=True)
    ...
    for sample in raw_samples:
        ...
        try:
            ...
            # Fusion + Attention 特征
            fusion_attn_feat = feature_extractor.extract_fusion_attention_features(emg_aligned, kin_aligned)
            
            # 合并特征
            feat = np.concatenate([emg_feat, kin_feat, fusion_attn_feat])
...
def train_synergy_n5_fusion_attention():
    print("="*60)
    print("HybridSCIMFP - n_synergies=5 + Fusion + Attention")
    print("="*60)
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_attention_predictions.csv', index=False)
    
    summary = pd.DataFrame([{
        'version': 'synergy_n5_fusion_attention',
        'n_synergies': 5,
        'mae': mae,
        'acc_1': acc,
        'acc_pm1': acc_pm1,
    }])
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_attention_summary.csv', index=False)
...
if __name__ == '__main__':
    result = train_synergy_n5_fusion_attention()
