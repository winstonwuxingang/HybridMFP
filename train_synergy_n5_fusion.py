# -*- coding: utf-8 -*-
"""
HybridSCIMFP - 策略3变体 + 层次化融合 (n_synergies=5)
集成train_v2.py的层次化融合网络
"""
import os
import sys
import numpy as np
...
class Config:
    ...
    OUTPUT_DIR = "./outputs/synergy_v1.0_n5_fusion"
    TARGET_LENGTH = 100
...
class HierarchicalFusionFeatureExtractor(nn.Module):
    ...
    def forward(self, emg_seq, kin_seq):
        ...
        返回:
            features: (B, hidden_dim * 2) 层次化融合特征
        """
        # 投影到统一维度
...
class FeatureExtractor:
    ...
    def extract_fusion_features(self, emg, kin):
        ...
        with torch.no_grad():
            ...
            fusion_feat = self.fusion_extractor(emg_tensor, kin_tensor)  # (1, hidden_dim*2)
...
def load_and_extract_features():
    print("="*60)
    print("加载数据并提取特征（n_synergies=5 + Fusion）...")
    print("="*60)
...
def train_synergy_n5_fusion():
    print("="*60)
    print("HybridSCIMFP - 策略3变体：n_synergies=5 + Fusion")
    print("="*60)
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_predictions.csv', index=False)
    
    summary = pd.DataFrame([{
        'version': 'synergy_n5_fusion',
        'n_synergies': 5,
        'mae': mae,
        'acc_1': acc,
        'acc_pm1': acc_pm1,
    }])
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_fusion_summary.csv', index=False)
...
if __name__ == '__main__':
    result = train_synergy_n5_fusion()
