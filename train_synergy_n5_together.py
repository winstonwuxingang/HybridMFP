# -*- coding: utf-8 -*-
"""
HybridSCIMFP - 策略3变体 + 一致性损失 (n_synergies=5)
集成train_v2.py的一致性损失
"""
import os
import sys
...
class Config:
    ...
    OUTPUT_DIR = "./outputs/synergy_v1.0_n5_together"
    TARGET_LENGTH = 100
...
def load_and_extract_features():
    print("="*60)
    print("加载数据并提取特征（n_synergies=5 + Consistency）...")
    print("="*60)
    ...
    subject_fma = {}
    subject_emg_kin = defaultdict(list)  # 存储原始EMG和KIN用于神经网络
...
def train_synergy_n5_together():
    print("="*60)
    print("HybridSCIMFP - 策略3变体：n_synergies=5 + Consistency")
    print("="*60)
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_together_predictions.csv', index=False)
    
    summary = pd.DataFrame([{
        'version': 'synergy_n5_together',
        'n_synergies': 5,
        'mae': mae,
        'acc_1': acc,
        'acc_pm1': acc_pm1,
    }])
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_together_summary.csv', index=False)
...
if __name__ == '__main__':
    result = train_synergy_n5_together()
