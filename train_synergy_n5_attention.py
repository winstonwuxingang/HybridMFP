# -*- coding: utf-8 -*-
"""
HybridSCIMFP - 策略3变体 + 多模态注意力 (n_synergies=5)
集成train_v2.py的时空注意力模块
"""
import os
import sys
import numpy as np
...
class Config:
    ...
    OUTPUT_DIR = "./outputs/synergy_v1.0_n5_attention"
    TARGET_LENGTH = 100
...
class SpatioTemporalAttention(nn.Module):
    ...
    def forward(self, fused_features):
        """
        参数:
            fused_features: (B, T, hidden_dim) 融合后的特征序列
        返回:
            attended: (B, T, hidden_dim) 带注意力增强的特征
        """
        # 时间自注意力 + 残差连接
        attn_out, _ = self.temporal_attention1(
            fused_features, fused_features, fused_features
        )
        fused_features = self.layer_norm1(fused_features + self.dropout(attn_out))
        
        # 第二个注意力层（跨模态增强）
        attn_out2, _ = self.temporal_attention2(
            fused_features, fused_features, fused_features
        )
        fused_features = self.layer_norm2(
            fused_features + self.dropout(attn_out2)
        )
...
class AttentionFeatureExtractor(nn.Module):
    ...
    def __init__(self, emg_dim=12, kin_dim=63, hidden_dim=64, num_heads=4, dropout=0.3):
        super().__init__()
    ...
    def forward(self, emg_seq, kin_seq):
        ...
        返回:
            features: (B, hidden_dim * 3) 注意力增强特征
        """
        # 投影到统一维度
        ...
        # 多模态融合
        concat = torch.cat([emg_feat, kin_feat], dim=-1)  # (B, T, hidden_dim*2)
        fused = self.fusion(concat)  # (B, T, hidden_dim)
        
        # 时空注意力
        attended = self.attention(fused)  # (B, T, hidden_dim)
        
        # 时间池化
        mean_pool = attended.mean(dim=1)  # (B, hidden_dim)
        max_pool = attended.max(dim=1)[0]  # (B, hidden_dim)
        std_pool = attended.std(dim=1)  # (B, hidden_dim)
        
        return torch.cat([mean_pool, max_pool, std_pool], dim=-1)  # (B, hidden_dim*3)
...
class FeatureExtractor:
    def __init__(self, fs_emg=1000, fs_kin=100, use_attention=True):
        self.fs_emg = fs_emg
        self.fs_kin = fs_kin
        ...
        if use_attention:
            self.attention_extractor = AttentionFeatureExtractor(
                emg_dim=12, kin_dim=63, hidden_dim=Config.HIDDEN_DIM, num_heads=4, dropout=0.3
            ).to(Config.DEVICE)
            self.attention_extractor.eval()
    ...
    def extract_attention_features(self, emg, kin):
        """提取注意力特征"""
        with torch.no_grad():
            emg_tensor = torch.FloatTensor(emg).unsqueeze(0).to(Config.DEVICE)  # (1, T, 12)
            kin_tensor = torch.FloatTensor(kin).unsqueeze(0).to(Config.DEVICE)  # (1, T, 63)
            
            attn_feat = self.attention_extractor(emg_tensor, kin_tensor)  # (1, hidden_dim*3)
            return attn_feat.squeeze().cpu().numpy()
...
def load_and_extract_features():
    print("="*60)
    print("加载数据并提取特征（n_synergies=5 + Attention）...")
    print("="*60)
    ...
    feature_extractor = FeatureExtractor(fs_emg=1000, fs_kin=100, use_attention=True)
    ...
    for sample in raw_samples:
        ...
        try:
            ...
            # 注意力特征
            attn_feat = feature_extractor.extract_attention_features(emg_aligned, kin_aligned)
            
            # 合并特征
            feat = np.concatenate([emg_feat, kin_feat, attn_feat])
...
def train_synergy_n5_attention():
    print("="*60)
    print("HybridSCIMFP - 策略3变体：n_synergies=5 + Attention")
    print("="*60)
    ...
    df.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_attention_predictions.csv', index=False)
    
    summary = pd.DataFrame([{
        'version': 'synergy_n5_attention',
        'n_synergies': 5,
        'mae': mae,
        'acc_1': acc,
        'acc_pm1': acc_pm1,
    }])
    summary.to_csv(f'{Config.OUTPUT_DIR}/synergy_n5_attention_summary.csv', index=False)
...
if __name__ == '__main__':
    result = train_synergy_n5_attention()
